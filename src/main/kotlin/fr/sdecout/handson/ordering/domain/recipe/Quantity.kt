...
sealed class Quantity {
    ...
    companion object {
        ...
        fun milligrams(amountInMilligrams: Int): Quantity = WithUnit(amountInMilligrams, MILLIGRAMS)
        fun grams(amountInGrams: Int): Quantity = milligrams(amountInGrams * 1000)

        fun milliliters(amountInMilliliters: Int): Quantity = WithUnit(amountInMilliliters, MILLILITERS)
        fun centiliters(amountInCentiliters: Int): Quantity = milliliters(amountInCentiliters * 10)

        fun pieces(number: Int): Scalar = Scalar(number)
    ...
    data class Scalar(override val amount: Int): Quantity() {
        ...
        override operator fun times(factor: Scalar) = pieces(this.amount * factor.amount)
